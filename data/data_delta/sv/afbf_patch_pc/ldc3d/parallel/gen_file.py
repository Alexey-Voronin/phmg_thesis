...
gen_file = lambda N, t, k, r, p_sweeps, h_sweeps, h_cycles : \
rf"""#!/bin/bash
#SBATCH --mem=246G 
#SBATCH --nodes={N}
...
srun python ../solver.py {k} {r} {p_sweeps} {h_sweeps} {h_cycles} 
"""
...
with open("job_map.log", 'a') as log_file_handle:
    ...
    for r, (nodes, tasks) in zip(range(1,3),         # refinements
                                [(6, 12), 
                                 (32, 6)
                                 ]): # (nodes, tasks per node)
        for k in range(3,11):
            file_path = f'r{r}_k{str(k).zfill(2)}_V{h_cycles}_p{p_sweeps}_h{h_cycles}.slurm'
            gen_file_content = gen_file(nodes, tasks, k, r, p_sweeps, h_sweeps, h_cycles)
